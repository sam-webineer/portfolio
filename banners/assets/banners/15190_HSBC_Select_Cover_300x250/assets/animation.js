...
function initAnimation() {
	...
	// Variables
	var fr2_txt_1_splits = new SplitText("#fr2_txt_1", {type:"chars", charsClass:'charsOne'});
	var fr2_txt_1_charArray = shuffleArray(fr2_txt_1_splits.chars);
	var fr2_txt_2_splits = new SplitText("#fr2_txt_2", {type:"chars", charsClass:'charsTwo'});
	var fr2_txt_2_charArray = shuffleArray(fr2_txt_2_splits.chars);
	...
	function StartScene() {
		...
		var all_elems = [nexus,fr1_txt,fr1_txt_1,fr2_txt_no,fr2_txt_1,fr2_terms,fr2_txt_2,fr2,list,'.item','.hex',logo,cta,cta_shine1]
		TweenMax.set([all_elems],{clearProps:"all"});
		TweenMax.set(['.charsOne'],{alpha:1});
	...
	function Frame1(anim,delay) {
		...
		.to(fr1_txt_1, 0, {alpha:1, text:'Love it?'}, delay)
		.to(fr1_txt_1, 0, {text:'Love it?<br>Cover it'}, '+=0.6')
	...
	function Frame2(anim,delay) {
		...
		// Out
		.staggerTo([fr1_txt,nexus], 0.8, {scale:0, alpha:0, ease:Back.easeIn.config(1.2)}, 0.1, delay)

		// In
		.fromTo(fr2_txt_no, 0.5, {alpha:0, scale:10}, {alpha:1, scale:1, ease:Power3.easeOut}, '-=0')
		.fromTo(fr2_txt_1, 0.5, {alpha:0, scale:1.5}, {alpha:1, scale:1, ease:Power3.easeInOut}, '-=0.45')
		.fromTo([fr2_terms], 0.5, {alpha:0, scale:1.5, transformOrigin:'50% -100% 0px', ease:Linear.easeNone}, {alpha:1, scale:1, ease:Power3.easeInOut}, 'scaleElems-=0.5')
		.fromTo([logo], 0.5, {alpha:0, scale:1.5, transformOrigin:'50% 250% 0px', ease:Linear.easeNone}, {alpha:1, scale:1, ease:Power3.easeInOut}, 'scaleElems-=0.5')
	...
	function Frame3(anim,delay) {
		...
		// Out
		.set(fr2_txt_2,{alpha:1})
		.to(fr2_terms, 0.4, {alpha:0, ease:Linear.easeNone}, delay)
		.staggerTo(fr2_txt_1_charArray, 0.23, {alpha:0, ease:Linear.easeNone}, 0.02, '-=0.4')

		// In
		.staggerFromTo(fr2_txt_2_charArray, 0.25, {alpha:0}, {alpha:1, ease:Linear.easeNone}, 0.02, '-=0.5')
		.to(fr2_offer, 0.4, {alpha:1, ease:Linear.easeNone}, '-=0.3')

		.to(counter, 0.4, {val:0.6, roundProps:"val", ease:Linear.easeNone, onUpdate:function(){  
			document.getElementById("counter").innerHTML = counter.val
		}}, '-=0.6')
		.to(fr2_txt_no, 0.2, {webkitFilter:"blur(5px)"}, '-=0.7')
		.to(fr2_txt_no, 0.2, {webkitFilter:"blur(0px)"}, '-=0.4')
	...
	function Frame4(anim,delay) {
		...
		// Out
		.set(list,{alpha:1})
		.to([fr2_terms], 0.4, {alpha:0, ease:Power2.easeOut}, delay)
		.to([logo,fr2], 0.7, {rotation:-45, scale:2.6, left:-253, top:-93, ease:Back.easeIn.config(1.2)}, '-=0.2')
		.to([logo,fr2_txt_2,fr2_offer], 0.2, {alpha:0, ease:Linear.easeNone}, '-=0.7')
		.set(nexus, {scale:1, alpha:1, left:-215})
		.set(fr2_txt_no, {alpha:0})
		
		// In
		.staggerFromTo('.item', 0.5, {alpha:0, y:'+=15'}, {alpha:1, y:'-=15', ease:Power2.easeOut}, 0.05, '-=0.2')
		.to(list, 0.6, {y:'-=34px', ease:Power1.easeInOut}, '-=0.7')
		.to(item2, 0.1, {color:'#db0011', ease:Power1.easeOut}, '=0')
		.to(list, 0.8, {y:'-=103px', ease:Power1.easeInOut}, '+=0.3')
		.to(item5, 0.1, {color:'#db0011', ease:Power1.easeOut}, '=0')
		.to(list, 0.5, {y:'-=33px', ease:Power1.easeInOut}, '+=0.4')
		.to(item6, 0.1, {color:'#db0011', ease:Power1.easeOut}, '=0')
	...
	function Frame5(anim,delay) {
		...
		.set(logo, {top:35, left:80, width:139, height:30})
		.set('.hex', {alpha:0})
		.to(list, 0.8, {y:'+250', alpha:0, ease:Back.easeIn.config(1.8)}, delay)
		.to(nexus, 1, {rotation:180, width:60, height:30, top:35, left:80, ease:Power2.easeInOut}, '-=0.5')

		// In
		.staggerTo([logo,fr4_txt_1,fr4_txt_2,cta,terms], 0.8, {alpha:1, ease:Linear.easeNone}, 0.08, '-=0.2')
		.to(cta_shine1, 1.2, {x:200, skewX:'-25', alpha:0.9, ease:Power2.easeOut}, '-=0.2')
